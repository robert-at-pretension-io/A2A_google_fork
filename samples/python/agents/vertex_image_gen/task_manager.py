...
class AgentTaskManager(InMemoryTaskManager):
    ...
    async def _stream_generator(
        ...
        prompt = self._get_user_prompt(task_send_params)
        try:
            ...
            result = self.agent.invoke(prompt, task_send_params.sessionId)
    ...
    def _get_user_prompt(self, task_send_params: TaskSendParams) -> str:
    ...
    def _create_artifacts_from_result(self, result: Dict[str, Any]) -> List[Artifact]:
        ...
        if "prompt" in result:
            parts.append(TextPart(text=result["prompt"]))
            
        # Add image file part if present
